...
// ✅ CORS mejorado para que Flutter web pueda conectarse
app.use(cors({
    origin: '*',
...
// ⚠️ Raw body PRIMERO (webhook vajadzībām)
...
async function getCountryFromIP(ip) {
    ...
    try {
        ...
        if (isLocalIP) {
            ...
            if (publicIp) {
                ipToCheck = publicIp;
                console.log(`📍 Lokālā IP ${ip} → publiskā IP ${publicIp}`);
            }
        }
        const controller = new AbortController();
        const timeoutId = setTimeout(() => controller.abort(), 5000);
        const response = await fetch(
            `http://ip-api.com/json/${ipToCheck}?fields=status,country,countryCode`,
        ...
        if (data.status === 'success') {
            result = { country_code: data.countryCode, country_name: data.country };
            console.log(`🌍 Valsts: ${data.country} (${data.countryCode})`);
        } else {
...
async function initDB() {
    try {
        ...
        // ⚠️ Turpina strādāt pat ja BD nestrādā —
        // /create-checkout-session darbosies bez BD
    }
}
            ...
            html: `
                <div style="font-family: sans-serif; max-width: 500px; margin: auto;">
                    ...
                    <div style="
                        ...
                        background: #f0f8e8; border: 2px solid #4a7c59;
                        border-radius: 8px; padding: 20px; text-align: center;
                        color: #2d5a3d; margin: 20px 0;
                    ">${displayCode}</div>
...
// ✅ Šis strādā BEZ datubāzes — tikai Stripe
    ...
    if (event.type === 'checkout.session.completed') {
        ...
        if (email) {
            ...
            try {
                ...
                console.log(`💾 Saglabāts customers ID: ${customerId}`);

                await pool.query(
                    `INSERT INTO activations
                     (activation_code, customer_id, device_id, ip_address,
                      user_agent, country_code, country_name)
                     VALUES (?, ?, ?, ?, ?, ?, ?)`,
                    [activationCode, customerId, deviceId, ipAddress,
                        userAgent, country.country_code, country.country_name]
                );
                console.log(`💾 Aktivācijas kods saglabāts`);
                ...
                if (err.code === 'ER_DUP_ENTRY') {
                    ...
                    console.error('❌ Kļūda saglabājot datubāzē:', err.message);
                }
            }
        }
    }
...
app.post('/verify-code', async (req, res) => {
    ...
    console.log(`🔍 Kods: "${code}"`);

    if (code.length !== 12) {
        return res.json({ valid: false, message: `Kodam jābūt 12 zīmēm. Saņemts: ${code.length}` });
    }
        ...
        if (rows.length > 0) {
            ...
            await pool.query(
                ...
                [deviceId, ipAddress, userAgent,
                    country.country_code, country.country_name, code]
            );
...
app.post('/check-activation', async (req, res) => {
    ...
    try {
        const [rows] = await pool.query(
            `SELECT id FROM activations WHERE device_id = ? AND is_used = TRUE LIMIT 1`,
            [deviceId]
        );
...
if (process.env.NODE_ENV !== 'production') {
    ...
    app.get('/debug/stats', async (req, res) => {
        const [[tc]] = await pool.query('SELECT COUNT(*) as total FROM customers');
        const [[ta]] = await pool.query('SELECT COUNT(*) as total FROM activations WHERE is_used = TRUE');
